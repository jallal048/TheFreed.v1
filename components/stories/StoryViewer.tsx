...
const NsfwStoryGate: React.FC = () => {
  ...
  const handleVerifyClick = () => { closeStoryViewer(); onGoToSettings('privacy'); };
  return (
    <div className="absolute inset-0 bg-black/70 backdrop-blur-xl flex flex-col items-center justify-center text-center p-6 z-30">
      ...
      <button onClick={handleVerifyClick} className="mt-4 bg-white/20 hover:bg-white/30 text-white font-bold py-2 px-6 rounded-full">Verify Age in Settings</button>
    </div>
  );
};

const StoryViewerComponent: React.FC = () => {
  ...
  useEffect(() => { if (isStoryViewerOpen) { setCurrentCreatorIndex(storyViewerStartIndex); setCurrentStoryIndex(0); } }, [isStoryViewerOpen, storyViewerStartIndex]);
  ...
  useEffect(() => {
    if (isStoryViewerOpen && currentStoryItem) {
      ...
      if (isNsfwStory && !canViewNsfw) { setProgress(0); return; }
      const timer = setInterval(() => { setProgress(p => p + 100 / 50); }, 100);
      return () => clearInterval(timer);
    }
  }, [isStoryViewerOpen, currentStoryItem, markStoryAsViewed, isNsfwStory, canViewNsfw]);

  const goToNextStory = useCallback(() => {
    if (currentStoryIndex < currentStorySet.length - 1) { setCurrentStoryIndex(prev => prev + 1); }
    else if (currentCreatorIndex < storyViewerCreatorIds.length - 1) { setCurrentCreatorIndex(prev => prev + 1); setCurrentStoryIndex(0); }
  ...
  useEffect(() => { if (progress >= 100) { goToNextStory(); } }, [progress, goToNextStory]);
  useEffect(() => { setProgress(0); }, [currentStoryItem]);
  ...
  const handleNavigationClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (clickPosition > 0.5) { goToNextStory(); } else { goToPrevStory(); }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-lg z-50 flex items-center justify-center">
      <div className="relative w-full h-full max-w-md max-h-[95vh] aspect-[9/16] rounded-lg overflow-hidden" onClick={handleNavigationClick}>
        ...
        {currentStoryItem.media.type === 'image' && (<img src={currentStoryItem.media.url} className={`w-full h-full object-cover ${isNsfwStory && !canViewNsfw ? 'blur-2xl' : ''}`} />)}
        {currentStoryItem.media.type === 'video' && (<div className={`w-full h-full bg-gray-900 flex items-center justify-center text-white ${isNsfwStory && !canViewNsfw ? 'blur-2xl' : ''}`}>Video not implemented</div>)}
...
export default StoryViewerComponent;
