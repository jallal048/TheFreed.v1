...
const EditPostModalComponent: React.FC = () => {
  const { isEditPostModalOpen, closeEditPostModal, editingPost } = useModals();
  const { editPost } = useData();
  const [text, setText] = useState('');

  useEffect(() => { if (editingPost) setText(editingPost.text || ''); }, [editingPost]);
  if (!isEditPostModalOpen || !editingPost) return null;

  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); editPost(editingPost.id, text); closeEditPostModal(); };
