...
const PpvMessageModalComponent: React.FC = () => {
  ...
  useEffect(() => { if (!isPpvMessageModalOpen) setTimeout(() => { setLoading(false); setSuccess(false); }, 300); }, [isPpvMessageModalOpen]);
  const handlePaymentSubmit = async ({ saveCard: shouldSaveCard }: { saveCard: boolean }) => {
    if (!ppvMessageModalMessage) return;
    setLoading(true);
    if (shouldSaveCard && !currentUser?.savedCard) { await saveCard({ brand: 'Visa', last4: '4242' }); }
...
export default PpvMessageModalComponent;
