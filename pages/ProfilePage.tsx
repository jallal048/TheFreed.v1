...
const NsfwProfileFeedGate: React.FC = () => {
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-800/50 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-6 text-center flex flex-col items-center">
      ...
      <p className="text-gray-600 dark:text-gray-400 mt-1 max-w-md">{t('nsfw.profileGateDescription')}</p>
      <button onClick={() => onGoToSettings('privacy')} className="mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-6 rounded-full transition-colors">{t('nsfw.profileGateAction')}</button>
    </div>
  );
};

const ProfilePageComponent: React.FC<ProfilePageProps> = ({ creator, posts }) => {
  ...
  const visiblePosts = canViewAllContent ? posts : posts.filter(post => !post.isNsfw);
  ...
  return (
    <div className="container mx-auto max-w-7xl">
      <div className="mb-6">
        <button onClick={onGoToHome} className="flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white font-semibold transition-colors duration-200">
          <Icon name="arrow-left" className="w-5 h-5" />
          {t('profilePage.backToHome')}
        </button>
      </div>
      <div className="mb-8"><CreatorProfile creator={creator} posts={posts} /></div>
      {activeStories.length > 0 && (<div className="mb-8"><StoryHighlights creator={creator} stories={activeStories} /></div>)}
      {showOnboarding && <div className="mb-8"><OnboardingChecklist /></div>}
      <div className="flex-1 min-w-0 space-y-8">
        {showNsfwGate && <NsfwProfileFeedGate />}
        {visiblePosts.length > 0 ? (<Feed posts={visiblePosts} />) : (
          <div className="text-center py-16 bg-gray-50 dark:bg-gray-900 rounded-2xl">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">{isNsfwProfile && !canViewAllContent ? t('profilePage.nsfwBlurb') : t('profilePage.noPostsBlurb')}</p>
          </div>
        )}
      </div>
    </div>
